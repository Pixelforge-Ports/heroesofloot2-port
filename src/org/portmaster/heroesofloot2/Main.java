...
public class Main extends myCanvas {
    ...
    public static Lwjgl3ApplicationConfiguration configuration() throws Exception {
        int width = Integer.getInteger("heroesofloot2.width",640), height = Integer.getInteger("heroesofloot2.height",480);
        ...
        Path saves = Paths.get(System.getProperty("heroesofloot2.saves","saves")).toAbsolutePath();
        java.nio.file.Files.createDirectories(saves);
        Lwjgl3ApplicationConfiguration cfg = new Lwjgl3ApplicationConfiguration();
        cfg.setTitle("Heroes of Loot 2"); cfg.setWindowedMode(width,height);
        ...
        cfg.disableAudio(Boolean.getBoolean("heroesofloot2.noAudio"));
        cfg.setInitialVisible(!Boolean.getBoolean("heroesofloot2.hidden"));
        if (Boolean.getBoolean("heroesofloot2.fullscreen")) cfg.setFullscreenMode(Lwjgl3ApplicationConfiguration.getDisplayMode());
        return cfg;
    }

    public static void main(String[] args) throws Exception {
        VerifyGame.check(Paths.get(System.getProperty("heroesofloot2.jar","HeroesOfLoot2.jar")));
        System.out.println("Heroes of Loot 2 PortMaster host 0.1.0 | " + System.getProperty("os.arch") + " | 60 updates/s");
    ...
    @Override public void create() {
        ...
        myPreferences = new com.orangepixel.dungeon2.DesktopPreferences() {
            @Override public String getDefaultPreferencesDirectory() { return System.getProperty("heroesofloot2.saves","saves"); }
        ...
        Gdx.input.setInputProcessor((InputProcessor)Proxy.newProxyInstance(Main.class.getClassLoader(),new Class<?>[]{InputProcessor.class},(self,method,args)-> {
            ...
            if (method.getName().equals("keyDown") || method.getName().equals("keyUp")) {
                ...
                if (key == Input.Keys.RIGHT) mirror = Input.Keys.D;
                if (key == Input.Keys.UP) mirror = Input.Keys.W;
                if (key == Input.Keys.DOWN) mirror = Input.Keys.S;
                if (mirror != -1) delegate(input,method,new Object[]{mirror});
            }
        ...
        System.out.println("GAME_CREATE_OK offline; PortMaster keyboard input; saves=" + System.getProperty("heroesofloot2.saves","saves"));
    }
    ...
    @Override public void render() {
        ...
        frames++;
        int smoke = Integer.getInteger("heroesofloot2.smokeFrames",0);
        if (smoke > 0 && frames >= smoke) {
            String capture = System.getProperty("heroesofloot2.capture");
    ...
    private void saveProfile() {
        try {
            Field field = myCanvas.class.getDeclaredField("activePlayer");
            field.setAccessible(true);
            if (field.get(null) != null) myCanvas.saveSettings();
        } catch (ReflectiveOperationException e) { throw new IllegalStateException("Cannot save settings",e); }
    }
