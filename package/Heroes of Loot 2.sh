...
GAMEDIR="/${directory#/}/ports/heroesofloot2"
java_runtime="zulu17.54.21-ca-jre17.0.13-linux"
jar_filename="HeroesOfLoot2.jar"

cd "$GAMEDIR" || { pm_message "Heroes of Loot 2: game folder missing."; pm_finish; exit 1; }
...
fail() {
  pm_message "Heroes of Loot 2: $* See heroesofloot2/log.txt."
  sleep 5
  cleanup
  exit 1
}
...
[ -f "$GAMEDIR/$jar_filename" ] || fail "Copy your owned HeroesOfLoot2.jar to heroesofloot2/HeroesOfLoot2.jar."
...
"$JAVA_HOME/bin/java" -Xmx64m -cp runtime/heroesofloot2-host.jar org.portmaster.heroesofloot2.VerifyGame "$GAMEDIR/$jar_filename" || fail "Unsupported or damaged game JAR. Check the README checksum."
...
$ESUDO env "${display_env[@]}" "$weston_dir/westonwrap.sh" headless noop kiosk crusty_glx_gl4es \
  ...
  "-Dheroesofloot2.jar=$GAMEDIR/$jar_filename" "-Dheroesofloot2.saves=$SAVEDIR" \
  -Dheroesofloot2.fullscreen=true "${display_java[@]}" \
  -cp "$GAMEDIR/runtime/heroesofloot2-host.jar:$GAMEDIR/$jar_filename" org.portmaster.heroesofloot2.Main
